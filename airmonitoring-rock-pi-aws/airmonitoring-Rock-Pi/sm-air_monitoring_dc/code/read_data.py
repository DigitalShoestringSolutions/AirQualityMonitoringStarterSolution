...
vocTable = "vocTable"
interval = 0.5  
machinename = "machine1"           # How long we want to wait between loops (in seconds)

sensor = DFRobot_ENS160_I2C(i2c_addr = 0x53, bus = 7)
def setup():
...
#   print("Carbon dioxide equivalent concentration : %u ppm" %(sensor.get_ECO2_ppm))
  ...
  #msg_str = {"Time":datetime.datetime.now().strftime("%I%M%p %B %d %Y") ,"machinename":machinename, "VOC":str(sensor.get_TVOC_ppb),
  ...
  msg_str={"Time":datetime.datetime.now(tz=tz).isoformat(),"machinename":machinename,"VOC":str(sensor.get_TVOC_ppb),"CO2":str(sensor.get_ECO2_ppm),"AQI":str(sensor.get_AQI)}
